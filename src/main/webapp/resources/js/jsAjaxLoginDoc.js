...
function identification() {
    ...
    xmlHttp.open("POST", "http://localhost:8080/ProfileNutrition2_war_exploded/logindoc", true); // Открываем асинхронное соединение
        ...
        if (xmlHttp.readyState == 4) { // Ответ пришёл
            if(xmlHttp.status == 200) { // Сервер вернул код 200 (что хорошо)
                if(xmlHttp.responseText=="true"){
                    window.location.href = "http://localhost:8080/ProfileNutrition2_war_exploded/doctor";
                }
                else {
